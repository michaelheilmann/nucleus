    ...
    )
{
    ...
    // (1)
    status = Nucleus_Collections_PointerHashMap_initialize(&collection,
                                                           8,
                                                           NULL,
                                                           NULL,
                                                           NULL,
    ...
    // (2)
    Nucleus_Size size;
    status = Nucleus_Collections_PointerHashMap_getSize(&collection, &size);
    ...
    status = Nucleus_Collections_PointerHashMap_getCapacity(&collection, &capacity);
    if (Nucleus_Unlikely(status)) goto Exit;
    if (capacity < size)
    {
        status = Nucleus_Status_InternalError;
    ...
    // (4)
Exit:
    Nucleus_Collections_PointerHashMap_uninitialize(&collection);
    return status;
}
